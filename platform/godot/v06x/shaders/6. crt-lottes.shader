...
// warpX
uniform float warpX: hint_range(0.0, 0.125, 0.01) = 0;//0.0155;

// warpY
uniform float warpY: hint_range(0.0, 0.125, 0.01) = 0;//0.0205;
...
// shadowMask
uniform float shadowMask: hint_range(0.0, 4.0, 1.0) = 1.0;
...
uniform vec2 OutputSize = vec2(1728, 576); //vec2(1024, 896);
uniform vec2 TextureSize = vec2(576, 288);
uniform vec2 InputSize = vec2(576, 288);
...
void fragment() {
	...
	vec2 pos = UV.xy*(TextureSize.xy/InputSize.xy)*(InputSize.xy/TextureSize.xy);
